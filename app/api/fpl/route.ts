...
async function fetchTeam(entry: EntryData, myPoints: number): Promise<FplTeam | null> {
  ...
  const leagues: FplLeague[] = targeted.map(({ league: l, targetRank, page }, i) => {
  ...
  try {
    ...
    const teamResults = await Promise.all(
      TEAMS.map((t, i) => {
        ...
        return fetchTeam(entry!, pts).then((team): FplTeamWithKey | null => {
          if (!team) return null;
          const gapToTop100k = pointsAt100k != null ? pts - pointsAt100k : undefined;
          return { ...team, teamKey: t.key as TeamKey, gapToTop100k, gwHistory };
        });
      })
    );

    const teams = teamResults.filter((t): t is FplTeamWithKey => t !== null);
    ...
    return Response.json({ active: false, error: true, teams: [] });
  }
}
...
interface FplLeague {
  ...
  targetRank: number; gapToTarget?: number; pointsAtTarget?: number | null;
...
interface FplTeam {
  ...
  gapToTop100k?: number;
  gwHistory?: { event: number; rank: number }[];
  leagues: FplLeague[];
}
interface FplTeamWithKey extends FplTeam { teamKey: TeamKey; gapToTop100k?: number; gwHistory?: { event: number; rank: number }[] }
